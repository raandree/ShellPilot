...
The real value is used only to make the request.
...
never hoisted above it. The cache is what makes that free: a still-valid token
is served with no network call, so the per-iteration call costs nothing until the
...
- It matches the STRUCTURED status ($_.TargetObject.StatusCode -eq 401), not the
  ...
  Invoke-ShpWithRetry reading that status BEFORE the connection-level
  classifier, otherwise a 401 is ridden out as a network outage for the whole
...
- It is one-shot per iteration, reset after an iteration succeeds. The retry shape
  ...
  once. A second 401 with a token exchanged seconds ago is not expiry, and says
  so by naming Initialize-Shp.
- It excludes the alternative-backend API key. That bearer is the caller's own
  ...
  header or exchanging a Copilot token would turn a clear misconfiguration

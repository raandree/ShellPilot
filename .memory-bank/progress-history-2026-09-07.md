...
- ShellPilot is a Sampler-built PowerShell module (cmdlet prefix Shp) with 35
  ...
  GitVersion, and GitHub Actions CI. At verified `6318225`, both `main` and
  `origin/main` have published preview `0.4.0-preview0013`; stable `0.3.1`
  remains the latest non-prerelease on GitHub Releases and the Gallery.
- All 13 migration specs (002-014) are implemented, plus 015 (batch execution),
  ...
  028 (CI annotation formatter), and 030 (host request transport and conditional
  admission);
...
- Distribution decision 7 is closed: stable and prerelease Gallery packages
  and GitHub Releases are published. MIT was selected on 2026-09-07; the license
  addition is local until merged and published. Stable `0.4.0` is not released.
- Tool policy path and command scoping is implemented (spec 019). It does not
  provide native containment, gate MCP calls, or enforce Copilot content
  exclusions or enterprise MCP allowlists.
- Complete release guardrails and the accepted remaining tranche-one scope:
  F7/F8, F23, F6, F22, and F17. F14 needs a measured credential-only probe;
  absence of the configured in-memory token must not block independent work.
...
- 2026-09-07 - Verify release-readiness baseline at `6318225`: clean worktree,
  matching `main` and `origin/main`, and successful package, Windows, macOS,
  Ubuntu, and deploy jobs in run 34105577285. GitHub and Gallery APIs confirm
  preview `0.4.0-preview0013` and latest stable `0.3.1`. The maintainer selects
  MIT. Correct current release documentation and retain historical records.
  Work proceeds on `ai/release-readiness-tranche-one`; no remote writes.
...
- 2026-08-12 - Token protected at rest (spec 020), closing open decision #5.
  ...
  and user-only, upgraded IN PLACE with no re-auth, and the next unattended call
...
- 2026-07-12 - Resolved the `git pull` merge conflict on `main`. Local `main`
  ...
  a chronological union: activeContext.md focus is now gpt-5.6 pricing
  (newest) -> Usage.ContextTokens (relabelled "Preceding change") -> the
  read_file/context-
...
- 2026-07-12 - Fixed `Invoke-Shp` not reporting `CostUSD`/`Credits` for the
  `gpt-5.6` model family (`gpt-5.6-luna`, `gpt-5.6-sol`, `gpt-5.6-terra`). Cost
  is data-driven from `PriceTable.psd1` and the price-key lookup is an exact,
...
- 2026-06-11 - Added wiki-content generation so the deploy
  ...
  plus Generate_Wiki_Sidebar + Clean_Markdown_Metadata + Package_Wiki_Content) and
...
- 2026-06-11 - Surfaced the GitVersion build version in the GitHub Actions UI
  ...
  and deploy job NAMES embed `needs.build.outputs.fullSemVer` (deploy needs
  widened
...
- 2026-06-11 - Reversed the prior publish fix per the user: instead of removing
  ...
  tasks are exposed as `Task.*` aliases), and restored the wiki step in the
  publish workflow. Installed the dep (DscResource.DocGenerator 0.13.0) via
...
- 2026-06-11 - Replaced the Azure Pipelines CI (azure-pipelines.yml, deleted)
  ...
  and workflow_dispatch; kept the CHANGELOG paths-ignore and `v*/!v*-*` tag filter.
...
- 2026-06-11 - Removed the bordered-box (single-cell HTML table) around the
  README logo per user request; logo is now a bare floated `<picture>`
  (align=left on the `<img>`). Two-variant theme switch and `<br clear="left">`
  unchanged. User
...
- 2026-06-11 - Ended the recurring logo theme/contrast loop: replaced the
  README's theme-switching `<picture>` wordmark with ONE self-contained image
  on a white card (assets/shellpilot-logo.png). Root cause (from the user's
...
- 2026-06-09 - Added a native opt-in manage_todo_list tool + structured progress
  ...
  silent on the console under the default InformationPreference but readable
  by a host from $shell.Streams.Information, the robust replacement for
  -ShowThinking
...
- 2026-06-07: Side task (unrelated to ShellPilot) - scaffolded a new standalone
  Sampler module FileManagement in C:\FileManagement per user request. 5 public
  file-mgmt functions + 1 private helper, full CBH, Pester tests, QA gates. Build
  green: 61 tests pass, PSScriptAnalyzer clean, module compiled to 0.1.0. Two
  local commits on branch ai/qa-fixes (genesis on master). Not pushed.

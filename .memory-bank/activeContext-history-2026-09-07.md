...
Execute release readiness and the remaining accepted tranche-one slices on
`ai/release-readiness-tranche-one`, created from verified `6318225`.
`main` and `origin/main` both point to that commit; the initial worktree was
clean, with `ai/fix-child-provider-ci-tests` checked out at the same tip.

GitHub Actions [run 34105577285](https://github.com/raandree/ShellPilot/actions/runs/34105577285)
passed package, Windows, macOS, Ubuntu, and deploy at `6318225`. GitHub Releases
and the Gallery both publish `0.4.0-preview0013`; the latest non-prerelease is
`0.3.1`. These service facts supersede the old failed-run focus below.

The maintainer selected MIT in this session. Stage 1 adds the license, corrects
installation and release status, closes distribution decision 7, indexes
specification 030, and documents absent Copilot content-exclusion and enterprise
MCP-allowlist enforcement. Markdown diagnostics are being repaired without
changing historical meaning.

Next: source-export regression and 7.4 CI coverage, the credential-only F14
probe, then F7/F8, F23, F6, F22, and F17 in tested local commits. Use only an
already configured `SHELLPILOT_GITHUB_TOKEN` for F14; never send a prompt or
record credentials. An absent token blocks only that probe. F9 is deferred
pending sign-off; its existing measurement is 10,166 prompt tokens for 61
MCP tools with two offered.

`review: on`: one independent security review after the implementation slices
are green. No push, publication, PR, remote change, or release tag is authorized.

## Historical baseline

The sections below retain earlier context. Their former push permissions,
pending statuses, and next steps do not authorize work in this session.
...
coverage; all 16 tasks completed without errors/warnings at 08:22:30 UTC.
DeskPilot passed 108 approval contracts before review and all 117 after the
review-Minor matrix against the exact merged module. Merge commit `4ab9eed`
was pushed to `origin/main` and the remote tip was verified equal. No package
...
verified cleanup. The merged gate protects their integration with the newer
`edit_file` source; a new authenticated child proof was not run for the merge.

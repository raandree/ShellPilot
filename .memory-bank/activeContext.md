...
Conversation continuation is now implicit: Invoke-Shp seeds every call from the
running session chat by default and Clear-ShpChat is the explicit reset; the
unreleased -ContinueChat switch is gone. Streaming (-Stream) and the session
chat both work; next is interactive Start-ShpChat (REPL) on top of -Stream and
implicit continuation, then responses-shape streaming.
...
a session default model, multi-turn conversation continuation (implicit),
chat and responses completion, reasoning-effort and max-output-token control,
live token streaming (-Stream, chat shape), a tool-calling loop (web and file
...
- Current state: green - 16 tasks, 0 errors; 191 tests pass; coverage 58.06%.
- QA gates TestQuality and helpQuality are enabled; PSScriptAnalyzer is clean.

## Conversation continuation (implicit, done 2026-06-06)

- Invoke-Shp seeds every call from the module-scoped $script:ShpChat by default
  (empty on the first call, populated automatically after) so a follow-up
  prompt remembers the previous turn without any switch - the natural usage.
- The unreleased -ContinueChat parameter was removed: continuation is now the
  default. Clear-ShpChat is the explicit reset.
- Invoke-Shp -History <objects>: continue from an explicit history (the result's
  ...
  Precedence as the seed: -History > session chat > none.
- Every result still carries a History property (prior turns + this exchange).
...
1. Build interactive Start-ShpChat (REPL) on top of -Stream and the implicit
   session chat.

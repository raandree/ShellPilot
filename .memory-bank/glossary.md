...
| Context report | The local per-source attribution of a request's estimated outgoing Context tokens (`Get-ShpContextReport`, or `Invoke-Shp -ContextReport` on the result's ContextReport), measured with one named estimator and reconciling with its own total. | token breakdown, usage report, context dump |
| Context source | One accounted row of a Context report - System, Instructions, SkillCatalog, SkillBodies, ToolSchemas, Attachments, SessionChat, Prompt or ToolResults - in that fixed order; a row this module cannot size locally reports null and a reason rather than zero. | context bucket, category, segment |
| Tool-result spill | The opt-in write of an oversized Tool result to a caller-named root (`Invoke-Shp -ToolResultSpillRoot`) instead of truncating it; redacted on write, never discovered, never pruned, and never silently replaced by truncation when the write fails. | result cache, overflow file, dump |
| Spill handle | The bounded JSON record sent to the model in place of a spilled Tool result, carrying the path, SHA-256, full length and a preview of the stored content. | pointer, stub, reference |
| Spill root | The existing, caller-owned directory a Tool-result spill is written into; refused rather than created when it does not exist. | output folder, cache dir, temp root |
| Session checkpoint | One stored, completed Session chat written by Save-ShpChat: redacted turns, the model they belong to, a parent, a SHA-256, and any uncertain side effects. | snapshot, save point, backup |
| Checkpoint store | The caller-named file holding a schema version, a revision and the chain of Session checkpoints; never discovered, never pruned, refused rather than migrated when its version is unknown. | session file, history file, database |
| Uncertain side effect | A side-effecting Tool call whose Turn never wrote its conversation back, recorded on a Session checkpoint by identity only and reported - never replayed - on resume. | pending call, orphan call, unfinished work |
| Trace identity | The `traceId`, `spanId`, `parentSpanId`, `runId` and `turnId` stamped on every Event record, additive to the record envelope and absent when the caller supplies none. | correlation id, request id, tracking id |
| Span key | The stable name a span is derived from within a trace - `turn`, `iteration:3`, `tool:call-1`, `decision:call-1:pre`, `mcp:call-1`, `subagent:<name>` - hashed with the trace and run ids into a span id, so no table of spans exists anywhere. | span name, span label |
| Traceparent | The W3C `00-<32 hex trace>-<16 hex span>-<2 hex flags>` string a call continues (`Invoke-Shp -TraceParent`) or publishes (`$result.Trace.TraceParent`); malformed values are refused, never replaced with a fresh trace. | trace header, parent id, correlation header |
| Mapping version | The explicit, `developmental` version of the Event-stream-to-OpenTelemetry translation reported by `ConvertTo-ShpOtelTrace`; an unimplemented one is refused rather than approximated. | export version, schema version, otel version |
| Remote attachment | An MCP server attached over Streamable HTTP (`Register-ShpMcpServer -Url`) rather than started as a local stdio process. | hosted server, HTTP server, cloud MCP |
| MCP channel | The transport record every protocol call goes through - stdio reader/writer or a remote endpoint - so era detection, the tool list and a tool call have one implementation, not two. | connection, session, socket |
| Pinned address | The address set a remote MCP endpoint resolved to when it was approved; a redirect or later request resolving outside it is refused as a DNS rebind. | resolved IP, allow-listed IP |
| Credential callback | The caller-owned scriptblock (`Register-ShpMcpServer -CredentialCallback`) that mints a token bound to one remote MCP resource, invoked per request and never stored; the only authorization this client performs. | auth handler, token provider, credential store |
| Resource provenance | The source root, relative path, size, SHA-256 and trust recorded for every Skill, Instruction or Agent definition file, reported on the availability, load and result surfaces. | file metadata, audit record, manifest |
| Resource fingerprint | The SHA-256 of a resource file's bytes, recorded when it is catalogued and checked when its body is loaded; it proves the bytes did not change, never that they were ever safe. | checksum, signature, verification |
| Catalog-to-load mutation | A resource file whose bytes changed between being advertised by name and description and having its body read; refused visibly, and the one provenance failure that is a denial. | file change, race, swap |
| Allowed-tools narrowing | An experimental front-matter tool list on a Skill or Instruction, intersected with what the turn already offered and applied after every other gate; it can only remove reach, never add it. | tool grant, permission list, capability |
| Subagent | A bounded child turn dispatched by `Invoke-ShpSubagent` from an explicit agent definition file, running under a strict subset of its parent's capability and returning an answer with evidence rather than a transcript. | sub-task, worker, delegate, child job |
| Attenuation | The rule that a Subagent's tools, switches, Tool policy, reach, redaction, execution contract and backend are a strict subset of its parent's; a request to widen is refused, never dropped. | inheritance, delegation, permission passing |
| Tree ledger | The single shared budget record for a whole Subagent tree - spend, iterations, deadline, depth, fan-out and concurrency - held by reference so a budget cannot be recovered by splitting it across more children. | budget pool, quota, allowance |
| Agent definition | The explicit, validated and fingerprinted `*.agent.md` file a Subagent is dispatched from; its body becomes the child's appended system prompt and its `tools` list is a narrowing request. | agent config, persona file, role file |

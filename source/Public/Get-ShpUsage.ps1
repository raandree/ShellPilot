function Get-ShpUsage {
    ...
    .DESCRIPTION
        ...
        prompt text, prompt and completion token counts, the estimated USD cost
        and credit count, the number of tool-calling iterations and tool calls,
        the finish reason and the wall-clock duration. Use it to analyse how many
        tokens and credits a PowerShell session has spent. With -Summary it
        returns a single aggregate object (totals plus a per-model breakdown)
        instead of the individual records. The log is scoped to the current
        PowerShell session and is not persisted to disk; reset it with
        Clear-ShpUsage.

    .PARAMETER Summary
        Return one aggregate object - total calls, prompt/completion/total
        tokens, total cost in USD, total credits, and a per-model breakdown -
        instead of the individual per-call records.
